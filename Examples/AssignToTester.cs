...
namespace Alvao.Helpdesk
{
    class AssignToTester : ITicketAutoAction
    {
        ...
        {
            if (!properties.Contains("tHdTicketCust.solvedInVersion")) return;
            ...
            // Values check.
            if (!String.IsNullOrEmpty(solvedInVersion) && section == Settings.ATT_ServiceName && stateId == Settings.ATT_ControlStateId)
            {
                // Get solver.
                int? newSolverId = con.QueryFirstOrDefault<int?>($@"SELECT TOP 1 TRP.liRolePersonPersonId newSolverId
                        FROM tRolePerson TRP JOIN tRole TR ON TRP.liRolePersonRoleId=TR.iRoleId WHERE TR.sRole = N'{Settings.ATT_SolverGroupName}'", transaction: trans);
